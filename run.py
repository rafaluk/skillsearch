...
from utils.utils import calculate_time, read_skills, save_offer_output, save_skills_output, save_links
import logging
import argparse
...
def run(timestamp, phases=[1,2,3,4]):
    ...
    if 1 in phases:
        job_links = run_link_extraction(driver, timestamp)

    # PHASE 2: OFFER EXTRACTION - extract content from all offers from links and save their content in Offer data model
    if 2 in phases:
        job_offers = run_offer_extraction(driver, job_links, timestamp)

    # PHASE 3: SKILL FINDING - get skills from offers in OfferWithSkills data model
    if 3 in phases:
        job_offers_with_skills = run_skill_finding(job_offers, timestamp)
...
def run_link_extraction(driver, timestamp):
    ...
    filename = "outputs/extracted_links" + timestamp + ".csv"
    save_links(filename=filename, links=job_links)
...
@calculate_time
def run_skill_finding(job_offers, timestamp):
    ...
    save_offer_output(filename=filename, offers=job_offers_with_skills)
...
def run_skill_aggregation(job_offers_with_skills, timestamp):
    ...
    save_skills_output(filename=filename, skills=skills)
...
if __name__ == '__main__':
    ...
    current_run_timestamp = datetime.now().strftime('_%H-%M-%S_%d-%m-%Y')
    ...
    if args['phases'] is not None:
        run(timestamp=current_run_timestamp, phases=args['phases'])
    else: 
